...
namespace NasuTek.DevEnvironment.Extensibility.Workbench
{
    public partial class DevEnvDocument : DevEnvPane {
        ...
        private void DevEnvDocument_FormClosing(object sender, FormClosingEventArgs e) {
            if (!Dirty) return;
            var retVal = MessageBox.Show("Do you want to save \"" + Text.Substring(0, Text.Length - 2) + "\"?", DevEnv.Instance.ProductName, MessageBoxButtons.YesNoCancel, MessageBoxIcon.Warning);
